...
class FinishReasonChoiceStrategy(ChoiceStrategy):
    ...
    def handle_execute_code_finish_reason(self, bot_backend: BotBackend, history: List, whether_exit: bool):
        ...
        try:
            ...
            history.append([None, f"GPT生成了错误的函数参数: {bot_backend.function_args_str}"])
            whether_exit = True
        except KeyError as key_error:
            history.append([None, f'后端键错误: {key_error}'])
            whether_exit = True
        except openai.error.RateLimitError:
            history.append([None, "API速率限制错误。请稍后重试。"])
            whether_exit = True
        except openai.error.APIError as api_error:
            history.append([None, f"OpenAI API错误: {api_error}"])
            whether_exit = True
        # except requests.exceptions.RequestException as req_error:
        #     history.append([None, f"网络请求错误: {req_error}"])
        #     whether_exit = True
        except Exception as e:
            history.append([None, f'后端错误: {e}'])
            whether_exit = True
        return history, whether_exit
